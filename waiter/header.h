...
void initSem()
{
    ...
    if (-1 == semctl(semID, WAITER, SETVAL, 4))
    {
        perror("Blad inicjowania wartosci semafora lokaja");
        exit(EXIT_FAILURE);
    }
}
...
void waitSem(int number)
{
    struct sembuf semOperation;
    semOperation.sem_num = number;
    semOperation.sem_op = -1;
    semOperation.sem_flg = 0;
    while (semop(semID, &semOperation, 1) == -1)
    {
        if (errno == EINTR)
        {
            continue;
